...
class BuildConstantsGeneratorPlugin : Plugin<Project> {
    ...
    private fun generateConstants(
        ...
        val regularPluginAliasToIdMap = pluginAliasToIdMap.filterKeys { it !in conventionPluginIds }
        ...
        return buildString {
            ...
            conventionPluginIds.sorted().forEach { id ->
                appendLine("            const val ${toConstName(id).replace('-', '_')} = \"$id\"")
            }
            appendLine("        }")
            appendLine("    }")
            appendLine()
